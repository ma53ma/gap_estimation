...
namespace gap_estimation {
    ...
    GapUtils::GapUtils(const GapEstimatorConfig& cfg) {
        cfg_ = & cfg;
    }

    std::vector<gap_estimation::Gap> GapUtils::hybridScanGap(boost::shared_ptr<sensor_msgs::LaserScan const> sharedPtr_laser, geometry_msgs::PoseStamped final_goal_rbt)
    {
        ...
        for (std::vector<float>::size_type it = 1; it < stored_scan_msgs.ranges.size(); ++it)
        {
            ...
            if (last_scan < max_scan_dist != scan_dist < max_scan_dist)
            {
                ...
                if (prev_lgap)
                {
                    ...
                    prev_lgap = false;
                    gap_estimation::Gap detected_gap(frame, gap_ridx, gap_rdist, false, half_scan);
                    detected_gap.addLeftInformation(it, scan_dist);
                    detected_gap.setMinSafeDist(min_dist);
    ...
    std::vector<gap_estimation::Gap> GapUtils::mergeGapsOneGo(
        boost::shared_ptr<sensor_msgs::LaserScan const> sharedPtr_laser,
        std::vector<gap_estimation::Gap>& raw_gaps)
    {
        //double start_time = ros::Time::now().toSec();
        std::vector<gap_estimation::Gap> simplified_gaps;
