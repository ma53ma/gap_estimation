...
namespace gap_estimation {
	...
	std::vector< std::vector<float>> obtainGapPoints(std::vector<gap_estimation::Gap> gaps, std::string ns, bool previous) {
	...
	vector<vector<double>> GapAssociator::obtainDistMatrix(std::vector<gap_estimation::Gap> observed_gaps, 
	...
	void GapAssociator::assignModels(std::vector<int> association, 
		...
		for (int i = 0; i < observed_gap_points.size(); i++) {
			...
			if (i % 2 == 0) {  // curr left
				observed_gaps[int(std::floor(i / 2.0))].right_model = new cart_model("right", *model_idx, init_r, init_beta, v_ego);
			} else {
				observed_gaps[int(std::floor(i / 2.0))].left_model = new cart_model("left", *model_idx, init_r, init_beta, v_ego);
			}
			*model_idx += 1;
		}
